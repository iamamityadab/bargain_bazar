...
if(btnval.equalsIgnoreCase("Search"))
{
	try
	{
		...
		ResultSet rs=smt.executeQuery("select * from manager where fname='"+t1+"'");
		%>
		<table border=2>
		<tr>
			<th>Fnm</th>
...
if(btnval.equalsIgnoreCase("update"))
{
	...
	try
	{
		Class.forName("oracle.jdbc.driver.OracleDriver");
		Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1523:xe", "distributor", "distributor");
		PreparedStatement psmt=conn.prepareStatement("update manager set location_id=?,lname=?,email=?,phone=? where fname=?");
		psmt.setString(1,t2);
		psmt.setString(2,t3);
		psmt.setString(3,t4);
		psmt.setString(4,t5);
	...
	if(btnval.equalsIgnoreCase("delete"))
	{
		try
		{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1523:xe", "distributor", "distributor");
			PreparedStatement psmt=conn.prepareStatement("delete from manager");
			psmt.executeQuery();
